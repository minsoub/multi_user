	...
	<div class="contents-framebox" id="contents_id">
		...
		<div class="contents-con-framebox">
			<div class="nav-box">
				...
				<div class="nav-list">
					<img alt="" src="/resource/images/sub/icon_home.png">&nbsp;HOME&nbsp;&nbsp;>&nbsp;&nbsp;사내방송&nbsp;&nbsp;>&nbsp;&nbsp;<b style="color:#000000;">감사나눔릴레이</b>
				</div>
				<div class="nav-title">감사나눔릴레이</div>
			</div>
			...
			<!-- 20190524 강정우 list화면 변경 -->
			<div class="internalmedia-box">
				<c:forEach var="kepcnFocusList" items="${kepcnFocusList }" varStatus="kepcnFocusListStatus">
					<div class="internalmedia-box01 fl_left" onclick="javascript:playPopup('${kepcnFocusList.re_form}', '${kepcnFocusList.num}');" style="cursor: pointer;">
						<div class="internalmedia-box02">
							<div class="internalmedia-time" >${fn:substring(kepcnFocusList.durationTime, 0, 8)}</div>
							<c:if test="${kepcnFocusList.thumb_log_path ne null }">
								<img src="${kepcnFocusList.thumb_log_path }" class="img_pointer"/>
							</c:if>
							<c:if test="${kepcnFocusList.thumb_log_path eq null }">
								<img src="/resource/images/broadcast_default.png" class="img_pointer"/>
							</c:if>
						</div>
						<div class="mbox01" style="padding: 15px 0px 0px 0px;">
						<c:set value="${fn:length(kepcnFocusList.subject) }" var="allLength" />
						<c:set value="${fn:substring(kepcnFocusList.subject, 0,24)}" var="cutSubject" />
							<span class="media_txt01_style">${allLength > 24 ? cutSubject.concat('...') : kepcnFocusList.subject }</span>
				       </div>                                
					</div>
				</c:forEach>
			</div>
