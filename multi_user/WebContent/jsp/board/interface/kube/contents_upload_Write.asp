...
'Response.Write FileName &" || "& saveFileName &" || "& uploadform.FileExtension
	...
	Else
		Set RefBoard = dbCon.execute("select ISNULL(MAX(SCF_SEQ), 0)+1 AS SCF_SEQ from tbSVC_CONTENTS_FILE")
		SCF_SEQ = RefBoard(0)
		
		streamSQL = "insert into tbSVC_CONTENTS_FILE (SCF_SEQ, SM_NO, SCF_fileName, SCF_orgFileName, SCF_ext, SCF_phycalPath, SCF_logicalPath, SCF_status, reg_user_pk, regDate)"
		streamSQL = streamSQL & "values ('" & SCF_SEQ & "', '1','"& saveFileName & "." & uploadform.FileExtension &"', '"& FileName &"', '" &File_ext& "', '" &FileSave& "', '" & "\" & NowThisYear & "\" & NowThisMonth & NowThisDay & "\" & saveFileName & "." & uploadform.FileExtension &"', '" & fileGubun & "', '" & tmp_EMPNO &"', GETDATE())"
