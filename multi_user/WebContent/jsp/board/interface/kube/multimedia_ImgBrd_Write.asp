...
'Code here that you want to catch errors from
    ...
    uploadform.DefaultPath = "E:\project_upload_test\multi_user\multiImgBrd\attatch_tmp\" & NowThisYear & "\" & NowThisMonth & NowThisDay

    '임시 폴더와 동일 폴더에 저장
	'strDirectory = "E:\VOD\KUBE\video_tmp\" & NowThisYear & "\" & NowThisMonth & NowThisDay & "\"
    strDirectory = "E:\project_upload_test\multi_user\multiImgBrd\attatch_tmp\" & NowThisYear & "\" & NowThisMonth & NowThisDay & "\"
...
'Response.Write FileName &" || "& saveFileName &" || "& uploadform.FileExtension
	...
	If Err.Number <> 0 Then
	   Response.Write "PROC_ERR"
		...
		Set RefBoard = dbCon.execute("select isnull(max(attatch_Seq), 0) + 1 as attatch_Seq from multi_Img_Board_Attatch")
		attatch_Seq = RefBoard(0)

		streamSQL = "insert into multi_Img_Board_Attatch(attatch_Seq, attatch_OrgName, attatch_PhyPath, attatch_LogPath)"
		streamSQL = streamSQL & " values ('" & attatch_Seq & "', '" & FileName & "', '" & strFile & "', '" & "\" & NowThisYear & "\" & NowThisMonth & NowThisDay & "\" & saveFileName & "." & uploadform.FileExtension &"')"
