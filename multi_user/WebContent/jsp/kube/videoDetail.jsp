	        ...
	        <div class="view_box01 fl_left">
	            <div id="playerDiv" style="width:100%;height: 600px;"> 
	                <c:if test="${videoInfo.conv_flag == '1'}">
			            <iframe src = ""  id="contentFrame" name="contentFrame" allowfullscreen="true" border="0" frameborder="0" scrolling="no" width="100%" height="100%"></iframe>
	                 </c:if>
...
<%-- e-러닝 Iframe Form 파라미터 --%>
<form name="contentsFrm" id="contentsFrm" method="post">
	<input type="hidden" name="playTime" id="playTime" value="${elearnUserMap.play_duration_sec eq null ? '0' : elearnUserMap.play_duration_sec}">
	<input type="hidden" name="videoPath" id="videoPath" value="${videoInfo.pds_log_path}">
	<input type="hidden" name="thumbPath" id="thumbPath" value="${videoInfo.thumb_log_path}">
	<input type="hidden" name="channel" id="channel" value="${videoInfo.channel}">
	<%-- <input type="hidden" name="gmEduComplete" id="gmEduComplete" value="${videoInfo.channel ne 'elearn' || eduPercent eq 100 ? true : false}"> --%>
	<input type="hidden" name="gmEduComplete" id="gmEduComplete" value="true">
</form>

<script type="text/javascript">
var gmPlayTime = $('#playTime').val();
var gmEduComplete = $('#elearnComplete_per').val() == 100 ? true : false;
	...
	if('${videoInfo.channel}' == 'elearn'){
		...
		if($('#elearnComplete_flag').val() == 0){
			...
			if('${elearnUserMap.play_duration_sec}' > 0){
				...
				$(obj)[0].currentTime = lmsTime;
				playTime($(obj)[0].currentTime, 1);
				//$(obj)[0].play();
			}
		}else{
...
function funElearnView(val){
	var frameObj = document.getElementById('contentFrame').contentWindow.document.getElementById('video1');
...
$(document).ready(function() {
	//20191112 Video Start
	if($('#contentFrame').length != 0){
		var f = document.contentsFrm;
		f.action = '/kube/kubeContentsFrame.do';
