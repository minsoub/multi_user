...
<div id="contents">
    ...
    <div class="fl_right" style="margin-bottom:10px">
        ...
        <c:forEach var="videoList" items="${videoList}" varStatus="videoStatus">
			...
			<c:if test="${!videoStatus.first}">
				...
				<tr id="commentTr${cha:seqEncode(tmp_vi_seq) }" style="display: none;">
					<td colspan="4" id="commentTbody${cha:seqEncode(tmp_vi_seq) }" style="text-align: left;"></td>
			    </tr>
				...
				<td class="report">
                    <div class="mbox01">
                        ...
                        <span class="media_txt01">등록일: ${cha:todate(videoList.reg_date, '-', '2')}</span>                 		
                    </div>
                </td>
                <td style="text-align:right; padding-right:25px;">
                    <span class="media_txt01">
                        조회수 : <fmt:formatNumber value="${videoList.visit_cnt}" pattern="#,###,###,###" /> &nbsp;&nbsp;/&nbsp;&nbsp; <font id="totCmmCnt${cha:seqEncode(videoList.vi_seq) }">댓글</font> : <font><fmt:formatNumber value="${videoList.comment_cnt}" pattern="#,###,###,###"/></font> &nbsp;&nbsp;/&nbsp;&nbsp; 다운로드 : <fmt:formatNumber value="${videoList.down_cnt}" pattern="#,###,###,###" />
                    </span><br><br>
                    <a class="btn_white" style="margin-left:10px;cursor: pointer;" onclick="javascript:funGetCommentList('${cha:seqEncode(videoList.vi_seq) }');">댓글현황</a><br><br>
			...
			<c:if test="${videoStatus.last}">
		   		...
		   		<tr id="commentTr${cha:seqEncode(videoList.vi_seq) }" style="display: none;">
					<td colspan="4" id="commentTbody${cha:seqEncode(videoList.vi_seq) }" style="text-align: left;"></td>
			    </tr>   
		   </c:if> 
...
function funGetCommentList(vi_seq_hash){
	...
	if($.trim($("#commentTbody" + vi_seq_hash).html()) != ''){
		$('tr[id^=commentTr]').hide();
		$('td[id^=commentTbody]').html('');
	}else{
