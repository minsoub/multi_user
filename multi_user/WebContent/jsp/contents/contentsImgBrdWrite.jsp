										...
										<form id="frm" name="frm" method="post">
											...
											<dl class="insert_ready">
												...
												<dd style="width: 200px;">
													<input type="radio" id="image_Secret_Open" name="img_Board_Secret" value="O" checked /><label for="image_Secret_Open">공개</label>
													<input type="radio" id="image_Secret_Close" name="img_Board_Secret" value="C" /><label for="image_Secret_Close">비공개</label>
												</dd>
											</dl>
											<dl class="insert_ready">
												<dt class="must-option"><label>제목</label></dt>
												<dd style="width: 200px;"><input type="text" id="img_Board_Title" name="img_Board_Title" value=""></dd>
											</dl>
											<dl class="insert_ready">
												<dt class="must-option"><label>내용</label></dt>
												<dd><textarea class="textarea-style" rows="10" cols="88" id="img_Board_Content" name="img_Board_Content" style="width: 618px;"></textarea></dd>
											</dl>
										</form>
...
function goSubmit(){
	...
	if(progressChk){
		...
		if(confirm('등록하시겠습니까?')){
			$.ajax({
				type : 'post',
				url : '/registContentsImgBrd.do',
				dataType : 'json',
				data : params,
				success : function(result){
					alert('등록되었습니다.');
					$('#frm').append('<input type="hidden" name="img_Board_Seq" value="'+result+'" />');
...
function fn_AttatchDel(obj){
	if($('#attatchArea dd').length == 1){
		...
		var params = {
			"img_Board_Seq" : 0,
			"attatch_Seq" : $('input[name=seqs]').eq(rowIdx-1).val()
		}
