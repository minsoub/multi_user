								...
								<fieldset>
									...
									<div class="row-group">
										<form id="frm" name="frm" method="post">
											<input type="hidden" name="img_Board_Seq" id="img_Board_Seq" value="${resultDetail.img_Board_Seq }" />
											<input type="hidden" id="pageNo" name="pageNo" value="${paging.pageNo }" />
											<dl class="insert_ready">
												<dt class="must-option"><label>공개여부</label></dt>
												<dd style="width: 200px;">
													<input type="radio" id="image_Secret_Open" name="img_Board_Secret" value="O" ${resultDetail.img_Board_Secret eq 'O' ? 'checked' : '' }><label for="image_Secret_Open">공개</label>
													<input type="radio" id="image_Secret_Close" name="img_Board_Secret" value="C" ${resultDetail.img_Board_Secret eq 'C' ? 'checked' : '' }><label for="image_Secret_Close">비공개</label>
												</dd>
											</dl>
											<dl class="insert_ready">
												<dt class="must-option"><label>작업명</label></dt>
												<dd style="width: 200px;"><input type="text" id="img_Board_Title" name="img_Board_Title" value="${resultDetail.img_Board_Title}"></dd>
											</dl>
											<dl class="insert_ready">
												<dt class="must-option"><label>내용</label></dt>
												<dd>
													<textarea class="textarea-style" rows="10" cols="88" id="img_Board_Content" name="img_Board_Content" style="width: 618px;"><c:out value="${resultDetail.img_Board_Content}" escapeXml="false" /></textarea>
												</dd>
											</dl>
										</form>
		...
		if(navigator.appName == 'Netscape' && ua.indexOf('Trident/7.0') != -1){
			...
			if(file != null){
				$('form[name="file_frm"]').eq((rowIdx-attatchedLng)).ajaxForm({
					url: "<%=Constants.IMGBRD_UPLOAD_ASP_CALL %>",
					type: "POST",
					crossdomain: true, 
...
function goSubmit(){
	...
	if(progressChk){
		if($.trim($('#img_Board_Title').val()) == ''){
			alert('제목을 작성해주세요.');
			$('#img_Board_Title').focus();
			return false;
		}
		...
		var params = {
				"img_Board_Seq" : $('#img_Board_Seq').val(),
				"attatch_Seqs" : attatch_Ids.join('|'),
				"img_Board_Secret" : $('input:radio[name=img_Board_Secret]:checked').val(),
				"img_Board_Title" : $('#img_Board_Title').val(),
				"img_Board_Content" : escapeHtml($('#img_Board_Content').text()),
		}
		
		if(confirm('수정하시겠습니까?')){
			$.ajax({
				type : 'post',
				url : '/modifyContentsImgBrd.do',
				dataType : 'json',
				data : params,
