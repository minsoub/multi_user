							...
							<fieldset>
								...
								<div class="row-group">
									<form id="frm" name="frm" method="post">
										<input type="hidden" id="img_Board_Seq" name="img_Board_Seq" value="${resultDetail.img_Board_Seq }">
										<input type="hidden" id="attatch_Seq" name="attatch_Seq" value="0" />
										<input type="hidden" id="pageNo" name="pageNo" value="${paging.pageNo }" />
										<dl class="insert_ready">
											<dt class="must-option"><label>작업명</label></dt>
											<dd style="width: 200px;">${resultDetail.img_Board_Title }</dd>
										</dl>
										<dl class="insert_ready">
											<dt class="must-option"><label>내용</label></dt>
											<dd>
												<textarea class="textarea-style" rows="10" cols="88" id="notice_Content" name="notice_Content" style="width: 618px;" readonly="readonly"><c:out value="${resultDetail.img_Board_Content}" escapeXml="false" /></textarea>
											</dd>
										</dl>
									</form>
...
function goUpdate(){
	$('#frm').attr('action', '/contentsImgBrdUpdate.do');
	$('#frm').submit();
}
...
function goDelete(){
	...
	if(confirm('삭제하시겠습니까?')){
		$.ajax({
			type : 'post',
			url : '/removeContentsImgBrd.do',
			dataType : 'json',
			data : params,
			success : function(result){
				if(result >= 0){
					alert('삭제되었습니다.');
					location.href="/contentsImgBrdList.do"
				}
			},
			error : function(e){
			}
...
function goDown(val){
	$('#attatch_Seq').val(val);
	$('#frm').attr('action', '<%=Constants.IMGBRD_DOWNLOAD_ASP_CALL %>');
	$('#frm').submit();
}
