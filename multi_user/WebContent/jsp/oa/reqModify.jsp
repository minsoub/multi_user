								...
								<fieldset>
									...
									<div class="row-group">
										<form id="frm" name="frm" method="post">
											...
											<input type="hidden" id="pageNo" name="pageNo" value="${paging.pageNo }" />
											<input type="hidden" id="sts"   name="sts" value="${resultDetail.sts}"/>

											<dl class="insert_ready">
												<dt class="must-option"><label>직원번호</label></dt>
												<dd style="width: 200px;">${resultDetail.userid }</dd>
											</dl>											
											<dl class="insert_ready">
												<dt class="must-option"><label>성명</label></dt>
												<dd style="width: 200px;">${resultDetail.writer }</dd>
											</dl>	
											<dl class="insert_ready">
												<dt class="must-option"><label>연락처</label></dt>
												<dd style="width: 200px;">${resultDetail.usertel }</dd>
											</dl>
											...
											<dl class="insert_ready">
												...
												<dd style="width: 300px;">
													<select id="stsCheck" name="stsCheck" onChange="javascript:StsChange(this.value);">
														<option value="0" <c:if test="${resultDetail.sts eq '0'}">selected</c:if>>신청</option>
														<option value="1" <c:if test="${resultDetail.sts eq '1'}">selected</c:if>>완료</option>
														<option value="2" <c:if test="${resultDetail.sts eq '2'}">selected</c:if>>취소</option>
													</select>
												</dd>
											</dl>												
											</c:if>
											...
											<dl class="insert_ready">
												<dt class="must-option"><label>상태</label></dt>
												<dd style="width: 300px;">
													<c:if test="${resultDetail.sts eq '0'}">신청</c:if>
													<c:if test="${resultDetail.sts eq '1'}">완료</c:if>
													<c:if test="${resultDetail.sts eq '2'}">취소</c:if>
												</dd>
											</dl>						
											</c:if>															
										</form>
									...
									<div class="btn-zone">
										<ul>
											...
											<c:if test="${isGetAdmin != null}">
											<li><input type="submit" class="search_btn" value="처리" onclick="goProcessSubmit(); return false;"></li>
											</c:if>
