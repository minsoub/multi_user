					...
					<div>
						<p class="inputDesign">
    						<select id="req_type"  name="req_type" style="width:148px;">
    							...
    							<option value="1" <c:if test="${paging.req_type eq '1'}">selected</c:if>>증명사진</option>
    							<option value="2" <c:if test="${paging.req_type eq '2'}">selected</c:if>>여권사진</option>
    							<option value="3" <c:if test="${paging.req_type eq '3'}">selected</c:if>>비자사진</option>
    							<option value="4" <c:if test="${paging.req_type eq '4'}">selected</c:if>>EP사진</option>
    							<option value="5" <c:if test="${paging.req_type eq '5'}">selected</c:if>>사원증사진</option>
    						</select>    							    							
                        </p>
...
function ExcelDownload()
{
	...
	if(xlsDown){
		...
		$('#frm').attr('action','/photoExcelDownload.do').attr('method', 'post').attr('target', 'ifrm').submit();
