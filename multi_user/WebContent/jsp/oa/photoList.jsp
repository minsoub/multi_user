							...
							<tbody>
								...
								<c:forEach var="item" items="${noticeList}" varStatus="status">
									...
									<c:if test="${isGetAdmin != null}">
										<c:set var="url" value="javascript:ViewDetail('${item.seq}', '${item.reg_id}');" />
									</c:if>
									<c:if test="${item.reg_id eq SESS_EMPNO}">
										<c:set var="url" value="javascript:ViewDetail('${item.seq}', '${item.reg_id}');" />
									</c:if>
									<tr>
										<td class="no-td">${totalCnt - (paging.perPageCnt*(paging.pageNo-1)) - status.index }</td>
										<td style="text-align: left;cursor: pointer;" onclick="${url}">${item.subject }</td>
