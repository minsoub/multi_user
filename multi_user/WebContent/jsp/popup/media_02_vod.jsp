...
<div class="popup_player_box">
	...
	<%-- 원고 다운로드 없음. 2016.12.20.기존 media_02_vod.asp 의 내용을 컨버팅만 해둠. 
	          작업할 경우, 디자인 추가 및 링크 asp 작업 필요. 
		<span class="title" style="padding-bottom:5px;">
		<a href="/popup/media_01_vod_content2.asp?Num=${Rs_num}" target="mb">영문</a> ㅣ <a href="/popup/media_01_vod_content3.asp?Num=${Rs_num}" target="mb">한글</a> ㅣ
			<c:if test="${j_pdsFile != ''}">
	            <a href="http://100.1.48.20/download.asp?file_dir=9-1&amp;file_name=${j_pdsFile}&amp;db=vod_s1&amp;Num=${Rs_num}&amp;cook_name=${sess_id}" target="_blank">원고다운</a>&nbsp;&nbsp;
            </c:if>
            <c:if test="${j_pdsFile == ''}">
            	<a href="javascript:alert('등록된 파일이 없습니다.');">원고다운</a>&nbsp;&nbsp;
            </c:if>
			ㅣ
			<c:if test="${j_mp3 != ''}">
			<a href="http://100.1.48.20/download.asp?file_dir=9-2&amp;file_name=${j_mp3}&amp;db=vod_s1&amp;Num=${Rs_num}&amp;cook_name=${sess_id}" target="_blank">MP3다운</a>&nbsp;&nbsp;
			</c:if>
			<c:if test="${j_mp3 == ''}">
			<a href="javascript:alert('등록된 파일이 없습니다.');">MP3다운</a>&nbsp;&nbsp;
			</c:if>
			|
			<a href="/popup/media_01_vod_content.asp?Num=${Rs_num}&amp;db=vod_s1" target="mb">정보</a> </span>
	--%>
