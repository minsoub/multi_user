...
setScript.prototype.fnVttToArr = function(vtt){
  ...
  $.each(_tmpArr, function(n, elem) {
    ...
    $.each(_lines, function(m, elem) {
      if(m == 0) _this.scriptArr[n].num = n;
      if(m == 0) _this.scriptArr[n].time = elem.replace(/\r/g,'');
      // else if(m == 99) _this.scriptArr[n].teller = elem.replace(/\r/g,'');
      else if(m == 1) _this.scriptArr[n].script1 = elem.replace(/\r/g,'');
      else if(m == 2) _this.scriptArr[n].script2 = elem.replace(/\r/g,'');
    });
